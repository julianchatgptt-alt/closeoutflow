...
export function renderIdentityEmail(input: IdentityEmailInput): EmailMessage {
  ...
  const actionHtml =
    actionUrl && label
      ...
      : "";
  const html = `<!doctype html><html lang="en"><body style="margin:0;background:#f1f3f6;color:#182230;font-family:Arial,sans-serif"><main style="max-width:600px;margin:0 auto;padding:32px 20px"><div style="background:#fff;border:1px solid #e0e4e9;border-radius:10px;padding:28px"><img src="https://closeoutflow.com/brand/email-header.png" width="160" height="40" alt="Closeout" style="display:block;width:160px;height:40px;border:0"><div style="height:1px;background:#e0e4e9;margin:24px 0"></div><h1 style="margin:0 0 14px;font-size:24px;line-height:1.3">${escapeHtml(subjects[input.kind])}</h1><p style="margin:0;line-height:1.6">${escapeHtml(detail)}${escapeHtml(expiry)}</p>${actionHtml}</div><p style="margin:18px 4px 0;color:#667085;font-size:13px">Closeout · <a href="https://closeoutflow.com" style="color:#315f98">closeoutflow.com</a></p></main></body></html>`;
