...
> **Status:** implementation complete; awaiting the independent Phase 4C audit.
> **Branch:** `codex/phase-4b-auth`
...
| 13 — Invitations | Complete | Added hashed, expiring, revocable, rotation-safe invitations, pending dedupe, management actions, Mailpit delivery, and rate limits. | Token/grant/lifecycle pgTAP, email unit, and browser tests pass. | None locally. | Configure AOD-3 before staging. |
| 14 — Invitation acceptance | Complete | Added existing/new-user continuations, locked email, verified email match, idempotent transactional acceptance, and membership reactivation. | Existing-user and new-user Mailpit Playwright journeys plus edge-case pgTAP pass. | None. | None. |
...
| 20 — Security, MFA, and sessions | Complete | Added password security, Supabase TOTP enrollment/challenge/removal, hashed one-time recovery codes, security events, current/global sign-out, and sensitive AAL2 checks. | Unit, pgTAP security, browser route, and server-guard checks pass. | Rich device/session inventory table deferred per AOD-9. | Confirm production MFA/session posture in AOD-5/AOD-7. |
...
| 25 — Full validation | Complete | Ran static, database, unit, server-only, build, five-project browser, accessibility, deterministic-type, live-runtime, secret, diff, and pre-commit checks. | See the exit review for exact results. | OAuth provider smoke is the only intentional local external-provider skip. | Run OAuth smoke after AOD-2 is configured. |
| 26 — Documentation and closeout | Complete | Added this register and the Phase 4B exit review; updated agent pointers to Phase 4C audit status. | Markdown formatting, links referenced by repository checks, Git diff, and final suite pass. | Phase 5 is explicitly not started. | Approve the Phase 4C audit and complete infrastructure actions before staging. |
...
There is no remaining founder action required for local Phase 4B validation.
