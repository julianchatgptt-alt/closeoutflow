...
Requirements: `create_project_requirement`, `update_project_requirement` (emits granular responsibility/due-date audit events from changed fields; **never touches `status`** — assignment completeness is derived, so responsibility changes require `requirement.assign` but no lifecycle transition), `mark_requirement_not_applicable` (the **only** path to `not_applicable_approved`; requires `requirement.set_not_applicable` + writes `requirement.marked_not_applicable`), `reverse_requirement_not_applicable` (returns the row to `active`), `archive_project_requirement`/`restore_project_requirement`, `reorder_project_requirements`, `bulk_update_project_requirements`, `apply_requirement_template`.
